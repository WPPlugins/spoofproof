...
//    $ResponseData .= "<img float=right src='".plugin_dir_url( __FILE__ )."img/".$singlepost['image_name']."' style='float: right; margin: 0.5em; border: 1px solid #ccc; border-radius: 10px;width: 100px; height: 100px; font-size: 10px;'>";
    $ResponseData .= "<img float=right src='".plugin_dir_url( __FILE__ )."img/".$singlepost['image_name']."' style='margin: 0.5em; border: 1px solid #ccc; border-radius: 10px;width: 100px; height: 100px; font-size: 10px;'><BR>\n";
...
//    $ResponseData="TEST<div style='display: block; min-height:120px;'><img float=right src='".plugin_dir_url( __FILE__ )."img/".$thedata['image_name']."' style='float: right; margin: 0.5em; border: 1px solid #ccc; border-radius: 10px;width: 100px; height: 100px; font-size: 10px;'><p>".$thedata['pass_phrase']."</p></div>";
...
?>
  <script type="text/javascript">
    ...
    {
      jQuery('#DisplayThumb').attr('src', '<?php echo plugin_dir_url( __FILE__ ); ?>img/'+jQuery('#LoginImage option:selected').text());
    }
  </script>
...
<?PHP /* $SelectedLoginImage = get_the_author_meta( 'LoginImage', $user->ID); */ ?>
                <img id="DisplayThumb" float=right src='<?php echo plugin_dir_url( __FILE__ ); ?>img/<?PHP echo $image_name; ?>' style='float: right; margin: 0.5em; border: 1px solid #ccc; border-radius: 10px; width: 100px; height: 100px; font-size: 10px;'>
                <Select name="LoginImage" id="LoginImage" onChange="LoginImageChange();">
<?php
    $sql = "Select * from ".$wpdb->prefix."users_images";
    $allposts = $wpdb->get_results($sql, ARRAY_A);
    foreach ($allposts as $singlepost) 
//    { echo '<option value="'.$singlepost['image_name'].'" title="'.plugin_dir_url( __FILE__ ).'img/'.$singlepost['image_name'].'" '.selected($singlepost['image_name'], $SelectedLoginImage).'>'.$singlepost['image_name'].'</option>'; }
//    { echo '<option style="background-image:url('.plugin_dir_url( __FILE__ ).'img/'.$singlepost['image_name'].'); ". "value="'.$singlepost['ID'].'" '.selected($singlepost['image_name'], $SelectedLoginImage).'>'.$singlepost['image_name'].'</option>'; }
    { 
      if ($singlepost['ID'] === $image) 
        {$Selected=" selected";}
      else
        {$Selected="";}
//      echo '<option style="background-image:url('.plugin_dir_url( __FILE__ ).'img/'.$singlepost['image_name'].'); " value="'.$singlepost['ID'].'"'.$Selected.'>'.$singlepost['image_name'].'</option>';      
...
?>
    <style type="text/css"> 
       ...
       .login h1 a {background-image: none, url('<?php echo plugin_dir_url( __FILE__ ); ?>img/wordpress-logo-plus.png')} 
    </style>
    <script type="text/javascript">
...
<?php
/*        url: "<?php echo plugin_dir_url( __FILE__ ); ?>admin-ajax.php", */
  }
}
...
?>
    ...
    <div id='SpoofProofResults' style='padding-bottom:10px;' >
        Enter your user name and click "<b>Next</b>".<BR><BR>
        You should see the text and the image you selected earlier, if you do not see your image and text, <font color=red><b>do not</b></font> continue.
        <BR>
    </div>
...
function SpoofProof_Next() 
{
  ...
  // Validate the token
  /* */
  $Token = ($_REQUEST['spoofproof_token']);
  ...
  /* */ 
    ...
    else
    {
       $ResponseData="<b>Make sure the following text and picture are yours, if they are not, do not log in!</b><BR><BR>".
                    "<div style='display: block; min-height:120px;'><img float=right src='".plugin_dir_url( __FILE__ ).
                    "img/".$thedata['image_name']."' style='float: right; margin: 0.5em;".
